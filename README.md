...
3. **Запуск обучения:**
   ...
   ```bash
   poetry run python -m road_segmentation.cli.train
   ```
   ...
   - Параметры модели (файл `last.ckpt`).

   Также будет создана символическая ссылка `outputs/last.ckpt`, указывающая на последний сохраненный чекпоинт в последней сессии обучения.
